...
void mp3PlayerDemo(const char *mp3file)
{
	...
	/* 配置ES8388芯片，使能DAC，使能输出通道 */
	es8388_adda_cfg(1, 0);	 /* DAC使能，ADC关闭 */
	es8388_output_cfg(1, 1); /* 通道1和通道2使能（耳机输出） */

	/* 调节音量，左右相同音量 */
	es8388_hpvol_set(mp3player.ucVolume);

	/* 配置ES8388音频接口为飞利浦标准I2S接口，16bit */
	es8388_sai_cfg(0, 3); /* fmt=0(I2S标准), len=3(16bit) */
